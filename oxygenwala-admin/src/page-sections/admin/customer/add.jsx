...
export default function Add(props) {
  ...
  const [cameraMode, setCameraMode] = useState("ocr");
  ...
  /* ================= CAMERA & OCR ================= */
  const captureAndProcess = React.useCallback(async () => {
    ...
    try {
      ...
      if (aadhaarMatch) {
        newFormData.aadhaarNumber = aadhaarMatch[0].replace(/\s/g, "");
        foundSomething = true;
      }

      const panMatch = text.match(/\b[A-Z]{5}[0-9]{4}[A-Z]{1}\b/i);
      if (panMatch) {
        newFormData.panNumber = panMatch[0].toUpperCase();
        foundSomething = true;
      }

      const gstMatch = text.match(/\b[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}\b/i);
      if (gstMatch) {
        newFormData.gstNumber = gstMatch[0].toUpperCase();
        foundSomething = true;
      }
      ...
      if (emailMatch) {
        newFormData.customerEmail = emailMatch[0];
        foundSomething = true;
      }

      if (foundSomething) {
        setFormData(newFormData);
        toast.success("Details extracted and auto-filled!", { theme });
      } else {
        toast.warn("No clear details found in image.", { theme });
      }

      // Also upload the photo
      await uploadCapturedImage(imageSrc, 'id_capture');
      setShowCamera(false);
    } catch (err) {
      toast.error("Failed to extract details from image.", { theme });
    } finally {
      setOcrProcessing(false);
    }
  }, [webcamRef, formData, theme]);
  ...
  const submitData = async () => {
    const validationRules = [
      ...
      { field: "alternateNumber", required: true, message: "Please enter alternate number." },
      {
        field: "alternateNumber",
        pattern: /^[6-9]\d{9}$/,
        message: "Please enter valid 10-digit alternate number.",
      },
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/adminApi/addCustomer`,
        formData
      );
      ...
      props.refreshTableData(response.data.customer);
      closeDialog();
    } catch (error) {
  ...
  return (
    <Fragment>
      <Dialog
        ...
        className="z-40"
      >
        <DialogHeader className="justify-center bg-gray-100 relative">
          ...
          {!showCamera && (
            <button
              ...
              className="absolute right-4 text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded border border-blue-200 hover:bg-blue-100 flex items-center gap-2"
            >
              <i className="fas fa-camera"></i> Auto-fill ID
            </button>
          )}
        </DialogHeader>
          ...
          {showCamera ? (
            <div className="flex flex-col items-center justify-center space-y-4 p-4">
              ...
              <div className="flex gap-4">
                ...
                <button
                  type="button"
                  onClick={cameraMode === 'ocr' ? captureAndProcess : captureAndUploadDoc}
                ...
                >
                  {ocrProcessing || uploading ? (
                    ...
                    <><i className="fas fa-camera"></i> {cameraMode === 'ocr' ? 'Capture & Extract' : 'Capture Document'}</>
                  )}
                </button>
              </div>
            </div>
