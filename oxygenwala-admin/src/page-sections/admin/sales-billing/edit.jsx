...
export default function Edit() {
  ...
  const [taxPercent, setTaxPercent] = useState(18); // default 18%
  ...
  useEffect(() => {
    ...
    if (id) {
        axios
          .get(`${import.meta.env.VITE_API_URL}/api/adminApi/getSalesBillById?id=${id}`, {
            headers: { Authorization: `Bearer ${localStorage.getItem("token") || ""}` }
          })
          .then((res) => {
            ...
            if (bill) {
                setFormData({
                    id: bill.id,
                    customerIdFk: String(bill.customerIdFk),
                    branchIdFk: String(bill.branchIdFk),
                    billingDate: dayjs(bill.billingDate).format("YYYY-MM-DD"),
                ...
                if (bill.items && bill.items.length > 0) {
                    setItems(bill.items.map(i => ({
                        id: i.id,
                        equipmentIdFk: i.equipmentIdFk ? String(i.equipmentIdFk) : "",
                        itemName: i.itemName,
                        quantity: i.quantity,
          ...
          .catch((err) => {
              ...
              toast.error("Failed to load invoice details.", { theme });
          });
    }
  }, [id, theme]);
  ...
  const addItemToCart = () => {
    ...
    const totalPrice = Number(currentItem.unitPrice) * Number(currentItem.quantity);

    setItems([
      ...items,
      { ...currentItem, id: Date.now(), totalPrice }
    ]);
  ...
  const submitData = async () => {
    ...
    const payload = {
      ...
      taxAmount,
      grandTotal,
      items: items.map(({ id, ...i }) => i) // backend will clear old and insert new
    };

    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/adminApi/updateSalesBill`, payload, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token") || ""}` }
      });
      toast.success("Sales invoice updated successfully.", { position: "top-center", theme });
      ...
      <CardBody className="bg-white px-4 pb-6 pt-0 text-blue-gray-600">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 pt-4 border-b pb-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData((prev) => ({ ...prev, paymentStatus: parseInt(e.target.value) }))}
              className="w-full border rounded-md p-2.5 text-sm focus:outline-none focus:border-blue-500 bg-white"
            >
        ...
        <div className="p-4 border rounded-xl bg-gray-50 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Item Name / Description *</label>
              <Input
                type="text"
        ...
        <div className="overflow-x-auto border rounded-xl mb-6">
          <table className="w-full min-w-[800px] table-auto">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan="5" className="p-4 text-center text-sm text-gray-500">No items added yet.</td></tr>
              ) : (
                items.map((i, index) => (
                  <tr key={i.id} className="hover:bg-blue-gray-50/20">
                    ...
                    <td className="border-b border-blue-gray-50 p-4">
                      <Typography variant="small" color="blue-gray">₹{i.unitPrice}</Typography>
                    </td>
                    <td className="border-b border-blue-gray-50 p-4">
                      <Typography variant="small" color="blue-gray" className="font-bold">₹{i.totalPrice}</Typography>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="flex justify-end mb-6">
            <div className="w-64 space-y-2 text-sm text-blue-gray-800">
                ...
                <div className="flex justify-between items-center border-b pb-2">
                    <span className="font-medium text-gray-500">Tax ({taxPercent}%):</span>
